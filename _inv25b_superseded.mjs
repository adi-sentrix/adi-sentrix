...
const FOREIGN=/\$\s?\d|\bUSD\b|inmoviliz|capital\s+(atrap|deten)|stock\s*usd|\d+\.\dx/i;  // capital/rotación (DOH "Nd" permitido)
// los gates de DOH/cobertura que pasan de AVISA → RESPONDE (Fix A · Fix C · spine 2.1b)
const SUP = ["qué SKU de Bosch tiene peor DOH","el SKU con más DOH","qué SKU tiene más DOH","qué productos tienen sobre-cobertura"];
console.log("══════ GATES DE DOH/COBERTURA SUPERSEDED · AVISA → RESPONDE (mundo nuevo · 2.5b) ══════\n");
let leak=0;
for (const q of SUP) {
  ...
  console.log(`  fuga ajena (capital/rotación/$): ${f?"🚨 SÍ":"✅ no"}\n`);
}
console.log(`RECONFIRMACIÓN: ${SUP.length} gates de DOH → RESPONDE · fugas ajenas: ${leak} ${leak?"🚨":"✅"}`);
process.exit(0);
