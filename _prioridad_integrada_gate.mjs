...
H("13 · segundas corridas vivas (autorizadas, 2 llamadas cada una): la prueba 1 cayó por «recuperas $22K» y «cambia el orden» — cerrados; la prueba 2 sirvió la reparación con dos errores que el muro no vio (caso al owner)");
{
  ...
  /* LA REPARACIÓN LLEVABA DOS ERRORES REALES QUE ENTONCES NADIE VIO (auditoría del Notario, owner 2026-09-14): «ese mismo grupo tiene markup
   * promedio 41.4%» (el grupo eran Falabella, Lider, Jumbo y Sodimac; el 41.4% es de las ocho bajo el benchmark) y «Cayendo (2 de 13 cuentas)»
   * (caen cuatro: Easy y Unimarc también). Hoy el muro los cobra —familia C de la auditoría— y el contrato sigue pasando; los dos falsos
   * positivos cerrados acá («recuperas $22K», «cambia el orden») no vuelven. */
  ok(!vetosDeRegistro(r1, { pregunta: Q, figs: M1.rp.ledger.figs, sitio: "reparacion" }).length && !M1.muro(r1).some((x) => x.kind === "total-mal-atribuido" || x.kind === "ranking-sin-cola"), "…y el contrato pasa; ni «recuperas $22K» ni «cambia el orden» vuelven", M1.muro(r1).map((x) => x.kind).join(","));
  ok(M1.muro(r1).map((x) => x.kind).sort().join(",") === "cifra-de-grupo-mal-repartida,conteo-de-lista-falso" && M1.muro(r1).some((x) => /«41\.4%».*«ese mismo grupo» remite/.test(String(x.detail))) && M1.muro(r1).some((x) => /«2 de 13 cuentas» caen: según la boleta son 4 de 13/.test(String(x.detail))), "★ del muro solo quedan los dos errores reales de la auditoría: el 41.4% de «ese mismo grupo» (son ocho) y «2 de 13 cuentas» cayendo (son cuatro)", M1.muro(r1).map((x) => x.kind + ": " + String(x.detail).slice(0, 90)).join(" | "));
  for (const [i, b] of [c1, r1].entries()) {
    ...
    ok(r.r.agente.estado !== "verde" && r.r.agente.vetos.some((v) => /«41\.4%»/.test(String(v))) && r.r.text.split(/\s+/).length >= 700, `★ con el ${i ? "segundo" : "primer"} borrador vivo como cerebro, el turno ya NO se sirve verde: el Notario detiene el 41.4% repartido y el usuario recibe el respaldo completo (${r.r.agente.estado}, ${r.r.text.split(/\s+/).length} palabras)`, JSON.stringify(r.r.agente.vetos).slice(0, 200));
  }
...
H("14 · universo de grupos · ordinales y rankings · «coincide en dos dominios» no es razón: los tres, sobre las respuestas reales");
{
  ...
  const M1 = muroDe(Q);
  for (const [i, b] of V6.borradores.entries()) ok(M1.muro(b.texto).filter((x) => x.kind === "cifra-de-grupo-mal-repartida").every((x) => /^«41[.,]4%»/.test(String(x.detail))) && M1.muro(b.texto).some((x) => x.kind === "cifra-de-grupo-mal-repartida") && !M1.muro(b.texto).some((x) => x.kind === "superlativo-no-sostenido") && !vetosDeRegistro(b.texto, { pregunta: Q, figs: M1.rp.ledger.figs, sitio: i ? "reparacion" : "cierre" }).map((x) => x.regla).includes("coincidencia-como-razon"), `el borrador ${i + 1} de la prueba 1 arde por el grupo SOLO en el 41.4% de «ese mismo grupo», y no por los otros dos`, M1.muro(b.texto).map((x) => x.kind + ": " + String(x.detail).slice(0, 60)).join(" | "));
}
...
H("15 · tercera corrida viva de la prueba 1 (autorizada, 2 llamadas): «carga baja» adjetivo · «de eso» pegado · «frenados» es capital · «carga» a secas — cerrados; la reparación se sirve entera");
{
  ...
  /* LA REPARACIÓN LLEVABA UN ERROR REAL QUE ENTONCES NADIE VIO (auditoría del Notario, owner 2026-09-14): «en esos mismos cuatro el precio de
   * lista está más pegado al costo que en los sanos (markup 41.4% contra 57.3% …)» — los cuatro son Falabella, Lider, Jumbo y Sodimac; el
   * 41.4% es el promedio de las OCHO bajo el benchmark. Hoy el muro lo cobra (familia C); los cuatro falsos positivos cerrados acá no vuelven
   * y el contrato sigue pasando. El turno ya no sirve la reparación entera: el Notario poda la frase del grupo y sirve el resto. */
  ok(reglas(r1, "reparacion").length === 0 && muro(r1).map((x) => x.kind).join(",") === "cifra-de-grupo-mal-repartida" && /«41\.4%».*«esos mismos cuatro» remite/.test(String(muro(r1)[0].detail)), "★ la reparación pasa el contrato, y del muro solo queda el error real de la auditoría: el 41.4% de «esos mismos cuatro» (son ocho)", [...muro(r1).map((x) => x.kind + ": " + String(x.detail).slice(0, 80)), ...reglas(r1, "reparacion")].join(" | "));
  ...
  ok(r.r.agente.estado !== "verde" && r.r.agente.vetos.some((v) => /«41\.4%»/.test(String(v))) && !/esos mismos cuatro/.test(r.r.text) && r.r.text.split(/\s+/).length >= 400, `★ con esa reparación como cerebro, el turno ya NO se sirve verde: el 41.4% repartido se detiene y lo servido no lo lleva (${r.r.agente.estado}, ${r.r.text.split(/\s+/).length} palabras)`, JSON.stringify(r.r.agente.vetos).slice(0, 200));
