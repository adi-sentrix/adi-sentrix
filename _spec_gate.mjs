...
const TESTS = [
  ...
  // COLAPSO DEL EJE (2026-08-30): antes el emisor decía «· escenario Bonanza» y el scrub del seam lo reescribía
  // a «· base real» — este chequeo exigía VER esa reescritura. Ahora el emisor no dice nada que scrubear (el
  // único mundo no se etiqueta) y los DOS caminos (16/0 y spec) emiten el mismo header sin etiqueta. La
  // propiedad que queda es la real: cero lenguaje de escenario ante el usuario.
  { n: "40 · overview sin lenguaje de escenario (el único mundo no se etiqueta)", spec: S({ operation: "overview", metric: "ventas", dimension: "cliente" }), ok: (r) => !/escenario|bonanza|tensi[oó]n|crisis/i.test(r.text) },
