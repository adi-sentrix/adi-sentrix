 ...
 * borra `ambient:true` de SentrixPanel.jsx y los 94 gates siguen verdes mientras el Perfil Ejecutivo se queda muda. El
 * usuario abre el Perfil Ejecutivo de Falabella, escribe «explicame esta ficha», y ADI no sabe qué ficha ni de quién.
 *
 ...
 *   [3] Cambiar de cara (Ficha → Capital) NO arrastra el contexto del Perfil Ejecutivo.
...
// el bloque de la llamada que montal Perfil Ejecutivo — el call site real, no una mención suelta en un comentario.
const BLOQUE_FICHA = (() => {
...
H("[0] POR QUÉ ESTE GATE · el Perfil Ejecutivo queda fuera del barrido que cubre a las demás caras");
{
  const n2 = componentIdsNivel2();
  ok(!n2.includes(FICHA),
    `el Perfil Ejecutivo NO está en componentIdsNivel2() (${n2.length} componentes con builder), así que §7f de _concordancia_cobertura_gate no la barre`,
  ...
  ok(!!VIEW_MANIFEST[FICHA] && !VIEW_MANIFEST[FICHA].builder,
    "el Perfil Ejecutivo se declara sin `builder` (su salida es la lectura del módulo, no un corredor de vista)");
...
{
  ok(!!BLOQUE_FICHA, "el panel montal Perfil Ejecutivo con useViewContext (no con un setUISignal a mano)");
  ...
  ok(/buildReadingFromSignals\(\s*buildClientContribSignals\(/.test(PANEL),
    "el panel derival Perfil Ejecutivo con buildReadingFromSignals∘buildClientContribSignals (lo mismo que afirma este gate)");
  ...
  ok(derivados > 0, `se derivó el contexto del Perfil Ejecutivo en ${derivados} combinaciones entidad × escenario`);
  ok(!faltas.length,
  ...
  for (const scn of ESCENARIOS) {
    for (const e of entidadesDe(scn)) {
      ...
      if (!/Perfil Ejecutivo/i.test(linea)) malLinea.push(`${scn}/${e}: la línea no nombral Perfil Ejecutivo`);
  ...
  ok(!malLinea.length,
    `la línea que lee el planificador nombral Perfil Ejecutivo y la entidad, y respeta el tope de ${VIEW_PLAN_LINE_MAX} caracteres`,
  ...
  ok(!malEvid.length,
    "la proyección que coacciona el plan lleva la entidad y la call que demuestral Perfil Ejecutivo (entityProfile)",
...
/* ══ [3] CAMBIO DE CARA ═════════════════════════════════════════════════════════════════════════════════════ */
H("[3] CAMBIAR DE FICHA A CAPITAL · sin arrastrar nada del Perfil Ejecutivo");
{
  const scn = "bonanza";
  ...
  ok(viewContextKey(vcFicha) !== viewContextKey(vcCapital), "la key del Perfil Ejecutivo y la de Capital nunca coinciden");
  ...
  ok(!/Perfil Ejecutivo/i.test(linea), "la línea del plan ya no mencional Perfil Ejecutivo", linea);
  ok(!(projectViewContextForCoercion(salida).entidades || []).length,
    "la entidad del Perfil Ejecutivo no viaja en la proyección de Capital");
...
H("[4] POR CTA Y A MANO · un solo contexto canónico, no dos parecidos");
{
  ...
  // sobre el panel entero, exigiendo que la desestructuración y el componentId del Perfil Ejecutivo estén pegados.
  ...
  ok(CTA_MISMA_LLAMADA.test(PANEL),
    "el CTA del Perfil Ejecutivo sale de la MISMA llamada a useViewContext que publica el ambiente (un solo `ctx`)");
  ...
  // …y el manifiesto lo dice: la vista del Perfil Ejecutivo no emite un contexto propio, lo emite su pieza.
  ok(VIEW_MANIFEST["ficha/otro/vista"] && VIEW_MANIFEST["ficha/otro/vista"]._emitidoPor === FICHA,
    "el manifiesto declara que la cara Perfil Ejecutivo se emite por su pieza — no hay un segundo contexto de ficha compitiendo");
