...
const leverBrecha = figOf(mBench, /Medida · cerrar brecha al piso/);
...
const leverCarga = figOf(mPal, /Medida · carga al target/);
...
ok("vs_presupuesto cuantifica cerrar el plan", /Cuánto vale:.*cerrar lo que falta al plan/.test(vPpto.text) && !!figOf(vPpto, /Medida · cerrar el plan/));
const inv = A({ schemaVersion: 1, operation: "inventory", metric: "capital", dimension: "bodega", focus: "frenado" }, {}, {});
const invLever = figOf(inv, /Medida · liberar /);
...
const leverScoped = figOf(t2, /Medida · cerrar brecha al piso/);
