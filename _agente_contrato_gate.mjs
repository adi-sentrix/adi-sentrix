...
H("7 · el mecanismo afirmado sin su sello arde, y la cuenta derivada que no cierra también — en el juez compartido");
{
  ...
  ok(!r("El mecanismo probado es la carga comercial; el precio de lista queda indicado (markup promedio 41.4% en los que caen contra 57.3% en los sanos) y el mix queda abierto: el dato no cruza cliente con familia.").includes("mecanismo-sin-sello")
    && !r("El precio de lista no parece ser el mecanismo principal, aunque sigue indicado.").includes("mecanismo-sin-sello") && !r("No es solo precio: la carga comercial también está probada.").includes("mecanismo-sin-sello")
  ...
  /* ── DESCRIBIR LO DEMOSTRADO SIN CAUSA NI JERARQUÍA (owner 2026-09-13, corrida 6) ────────────────────────────────
   * «sosteniendo contribución con mejor costo relativo» (los sanos solo están probados sobre el benchmark) y «la causa
   * dominante y probada es exceso de carga comercial» (probado que el mecanismo existe, no que explique la mayor parte). */
  const _m = (t) => vetosDeRegistro(t, { huellas, figs: [] }).map((v) => v.multa).join(" ");
  ok(r("Ayudan los otros 5: La Polar (34%), Hites (33%), Unimarc (32.5%), Easy (32%) y ABC (31%) — sobre el benchmark, sosteniendo contribución con mejor costo relativo.").includes("mecanismo-sin-sello") && /explicas un resultado por «con mejor costo relativo»/.test(_m("Los sanos sostienen contribución con mejor costo relativo.")),
    "★★★ «con mejor costo relativo» ARDE: explica el margen de los sanos por el costo (INDICADO) y el dato solo demuestra que están sobre el benchmark");
  ok(r("Easy y La Polar sostienen el margen gracias a su mix.").includes("mecanismo-sin-sello") && r("Los sanos ganan margen por un costo más bajo.").includes("mecanismo-sin-sello") && r("Falabella cae producto de un precio de lista pegado al costo.").includes("mecanismo-sin-sello"),
    "★ «gracias a su mix», «por un costo más bajo», «producto de un precio de lista…» también");
  ok(!r("Ayudan los otros 5: La Polar (34%), Hites (33%), Unimarc (32.5%), Easy (32%) y ABC (31%) — están sobre el benchmark.").includes("mecanismo-sin-sello") && !r("Seis de los ocho clientes bajo el benchmark están con carga sobre el nivel de referencia.").includes("mecanismo-sin-sello") && !r("Y de esos, 6 ceden margen por acciones comerciales.").includes("mecanismo-sin-sello"),
    "…lo demostrado a secas, «con carga sobre el nivel» (la huella) y «por acciones comerciales» (probado) pasan");
  ok(r("La causa dominante y probada es exceso de carga comercial; el precio de lista pegado al costo es una señal adicional aún no confirmada.").includes("jerarquia-causal-sin-medida"),
    "★★★ «la causa dominante y probada» ARDE: probado que el mecanismo existe, no que explique la mayor parte del efecto");
  ok(r("El mecanismo principal es la carga comercial.").includes("jerarquia-causal-sin-medida") && r("La brecha viene principalmente por acciones comerciales.").includes("jerarquia-causal-sin-medida") && r("La carga comercial explica la mayor parte de la brecha.").includes("jerarquia-causal-sin-medida") && r("Lo que más pesa es la carga comercial, no el precio.").includes("jerarquia-causal-sin-medida") && r("El precio de lista es una causa secundaria.").includes("jerarquia-causal-sin-medida"),
    "★ «mecanismo principal», «principalmente por», «explica la mayor parte», «lo que más pesa», «causa secundaria» también — con o sin huellas");
  ok(vetosDeRegistro("La causa dominante es la carga comercial.", {}).some((v) => v.regla === "jerarquia-causal-sin-medida"), "…y sin huellas en el contexto la jerarquía igual arde (ordenar causas sin medida es falso en cualquier turno)");
  ok(!r("La causa probada es exceso de carga comercial: 6 de los que caen tienen la carga por sobre el 3.5%.").includes("jerarquia-causal-sin-medida") && !r("La huella más clara es la carga comercial; el dato no mide qué parte de la brecha explica cada mecanismo.").includes("jerarquia-causal-sin-medida") && !r("No es la causa principal: es un mecanismo probado, sin medir su peso.").includes("jerarquia-causal-sin-medida") && !r("Podría ser el mecanismo principal, pero el dato no mide qué parte del efecto explica.").includes("jerarquia-causal-sin-medida"),
    "…«la causa probada», «la huella más clara», la negación y la hipótesis marcada pasan");
  /* ── UNIVERSOS CONSISTENTES · CADA SUBTOTAL CON SU UNIVERSO · LA PARTICIÓN MEDIDA (owner 2026-09-13, corrida en vivo) ── */
  {
    const { runPlan } = await import("./src/adi/oracle/toolRunner.js");
    const { TOOLS } = await import("./src/adi/oracle/toolRegistry.js");
    const { pasosDelContratoComercial } = await import("./src/adi/agente/contratoComercial.js");
    const rpC = runPlan({ intent: "answer", calls: pasosDelContratoComercial().map((s) => ({ tool: s.tool, args: s.args || {} })) }, { scenario: ESCENARIO_INICIAL, maxCalls: 8, preguntaUsuario: "x", registry: TOOLS });
    const figsC = (rpC.ledger || {}).figs || [];
    ...
    ok(!rc("El componente precio y costo pesa más que la carga: $4.4M contra $588K en las 5 cuentas materiales.").includes("jerarquia-causal-sin-medida") && !rc("De los $4.9M, $588K corresponden a carga excedida; el componente precio/costo, $4.4M, es el que más pesa.").includes("jerarquia-causal-sin-medida"),
      "★★ con la partición medida citada, «pesa más» en dinero es una jerarquía MEDIDA y pasa");
    ok(rc("El componente precio/costo pesa más.").includes("jerarquia-causal-sin-medida") && /La partición SÍ está en la boleta/.test(mc("El componente precio/costo pesa más.")) && /\$588K/.test(mc("El componente precio/costo pesa más.")),
      "★ …y sin citarla arde, con una multa que nombra la partición que sí está en la boleta ($588K · $4.4M)");
  }
  ok(!r("El desempeño general es positivo, impulsado principalmente por Lider, Jumbo, Falabella y Mercado Libre.").includes("jerarquia-causal-sin-medida") && !r("Y ese crecimiento está viniendo sobre todo de cuentas con margen bajo el promedio de tu cartera.").includes("jerarquia-causal-sin-medida") && !r("En Falabella el efecto carga es −1.0 pp contra un efecto costo de −0.5 pp: ahí la carga pesa más.").includes("jerarquia-causal-sin-medida") && !r("Esa es la pregunta que solo tú respondes, porque ningún número mide intención, y la causa de fondo —por qué esas 4 cuentas concentran carga y markup bajo a la vez— no está probada en el dato: solo localiza.").includes("jerarquia-causal-sin-medida"),
    "…el peso entre CUENTAS (una cifra), la parte del efecto medida (efecto carga contra efecto costo, en pp) y «la causa de fondo … no está probada» pasan");
