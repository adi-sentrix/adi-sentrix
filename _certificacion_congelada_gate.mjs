...
H("5 · carnadas: la certificación congelada es la RED de todo lo demás");
{
  ...
  await carnada("los playbooks desconectados del bucle",
    // (re-apuntada T5, 2026-09-05: el sitio pasa el hilo al detector — la carnada mide lo mismo de siempre)
    [[/  const playbook = \(\(\) => \{ try \{ return playbookPara\(q, \{ history \}\); \} catch \{ return null; \} \}\)\(\);/, "  const playbook = null;   // CARNADA"]],
    async (bucleMut) => {
      initTenant(TENANT_DEMO);
