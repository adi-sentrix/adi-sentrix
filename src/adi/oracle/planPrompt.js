...
calcular{operacion,insumos:[{entidad?,metrica}|{usuario:"la cifra CON su origen"}],objetivo?} — LA CALCULADORA del motor: la cuenta que la pregunta pide, ejecutada con su fórmula declarada. operacion: suma, resta, variacion_pct (de A a B), participacion (A sobre B), brecha_pp (tasa vs tasa), escalar (regla de tres), margen_objetivo ("¿qué falta para llegar a 25%?" → insumos:[{entidad:"X"}] + objetivo:{usuario:"25% y su origen"}). Cada insumo es una REFERENCIA (entidad+metrica del dato, o la cifra del usuario con su origen — jamás un número suelto): NUNCA hagas la aritmética vos mismo. Si las unidades no operan o los universos no reconcilian, la tool declina honesto.`;
