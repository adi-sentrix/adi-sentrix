...
export const MODES = [
  {
    ...
    narrate: "Dá el dato claro y cierra con un breve \"qué mirar/hacer\" (ver PEDIDO DE DATO / CAMPO CONCRETO abajo) — nunca el dato pelado sin lectura.",
  },
  {
    key: "diagnostico",
    whenToUse: "el usuario pide el PANORAMA — \"qué está pasando\", \"cómo viene el negocio\", \"resumen ejecutivo\", \"un diagnóstico\", \"dame un panorama\" — quiere ENTENDER la foto completa antes de decidir. NO es esto un \"cuéntame más\"/\"profundiza\" que sigue un tema YA establecido en el hilo (eso es seguimiento, aunque la frase suene a pedir más información) — diagnostico es para arrancar la foto completa DE CERO, no para seguir ahondando en lo que ya se venía hablando.",
    narrate: "Cuenta la HISTORIA completa en el arco de 3 movimientos (ver LA ESTRUCTURA abajo): qué pasa, por qué, qué hacer primero. Si además el pedido fue específicamente \"resumen ejecutivo\", sigue el contrato de 8 movimientos (ver RESUMEN EJECUTIVO abajo).",
  },
  {
    key: "decision",
    whenToUse: "el usuario pide UNA acción o prioridad — \"qué hago primero\", \"por dónde empiezo\", \"a cuál priorizo\", \"qué recomiendas\", \"cuál ataco primero\" — quiere LA DECISIÓN, no el panorama completo.",
    narrate: "Arranca DIRECTO por la acción — a lo sumo UNA frase de contexto antes, nunca el diagnóstico completo primero. Nombra el mecanismo real que el dato te dio (ver MECANISMO YA RESUELTO abajo) y su $ si lo tienes. Elige UNA acción y cierra ahí — no dejes 2-3 caminos abiertos esperando que el usuario decida por ti; si tu elección no es la de mayor monto, di el criterio explícito (ver SUPERLATIVOS CONSISTENTES en SAGRADO).",
  },
  {
    key: "simulacion",
    ...
    narrate: "Enmarca SIEMPRE como HIPÓTESIS (\"si bajas la carga al target, recuperarías $X — estimado\"), nunca como hecho consumado (ver SIMULACIÓN abajo). Nombra el supuesto, el efecto, dónde pega y el límite si el dato lo trae.",
  },
  {
    key: "seguimiento",
    whenToUse: "el usuario CONTINÚA la misma conversación sin agregar un ángulo nuevo — \"sí\", \"dale\", \"seguí\", \"profundiza\", \"eso mismo\", \"y\" — quiere MÁS de lo mismo, no un tema nuevo. Esto INCLUYE paráfrasis coloquiales COMPLETAS que combinan el marcador de continuación con un pedido de profundizar sobre lo YA dicho — \"dale, cuéntame más de eso\", \"dale, seguí\", \"bueno, profundiza en eso\", \"y, ¿qué más?\" — no busques SOLO la palabra suelta \"dale\": la frase completa (marcador + \"cuéntame/dime más\", \"segui[te]\", \"profundiza\") sigue siendo seguimiento del MISMO tema, NUNCA un pedido de panorama nuevo (eso sería diagnostico, y solo aplica si el usuario cambia de tema o pide explícitamente \"un resumen\"/\"cómo viene el negocio\" desde cero). Si la continuación en realidad trae una pregunta más específica (pide el cálculo → evidencia; pide priorizar → decisión; señala confusión → clarify), usa ESE modo en vez de seguimiento puro.",
    narrate: "NO reinicies el diagnóstico ni cambies de entidad/métrica/acción — mantén EXACTAMENTE la misma del hilo_reciente (ver SEGUIMIENTOS/deixis abajo) y profundiza UN nivel más (el siguiente detalle de la MISMA historia). Nunca vuelvas a explicar el contexto que ya diste ni reformules la respuesta anterior con otras palabras.",
  },
  {
    key: "evidencia",
    ...
    narrate: "Abre el cálculo: nombra las cifras autorizadas que se combinan y CÓMO (la resta/suma exacta ya permitida — ej. \"X% de margen menos tu benchmark de Y% = Z puntos de brecha\"). Gradúa cada afirmación: PROBADO (el dato la confirma directamente) / INDICADO (una señal, no cierra la causa) / ABIERTO (no se puede afirmar con este dato — dilo así, nunca lo inventes). No repitas la conclusión sin mostrar CÓMO se llega a ella — es exactamente lo que están pidiendo. Esta es LA MISMA gradación de honestidad que el punto (2) de LA ESTRUCTURA (narratePromptC.js) exige en CUALQUIER modo — acá solo se vuelve más explícita porque el pedido es justamente ver el cálculo.",
  },
  {
    key: "clarify",
    whenToUse: "el usuario señala CONFUSIÓN sobre lo que YA le dijiste — \"no entendí\", \"no entiendo\", \"no comprendo\", \"explícame más fácil/simple\", \"qué significa X\", \"a qué te refieres con X\", o repite casi la misma pregunta (señal de que no aterrizó, no de que cambió de tema).",
    narrate: `ESTO REEMPLAZA TODO EL ARCO para este turno — no la redactes de nuevo con otras palabras, re-enseña:
  · LA APERTURA (owner 2026-08-13): NUNCA abras con una contrapregunta tipo "¿qué parte no entendiste?"/"¿qué te genera confusión?" — un "no entiendo" pelado se responde RE-ENSEÑANDO de inmediato el mensaje central de tu respuesta anterior, en simple; repreguntar le cuesta un turno al usuario, y la pregunta guía del cierre (abajo) ya le ofrece profundizar. ÚNICA EXCEPCIÓN — MULTI-TEMA REAL: si tu respuesta anterior cubrió VARIOS temas distintos (entidades o métricas separadas, cada una con su propia lectura — una respuesta larga de UN solo tema NO cuenta), ahí sí puedes preguntar cuál de esos temas retomar, nombrándolos.
  · NO repitas el resumen ni la respuesta anterior reformulada — eso es exactamente lo que no funcionó.
  · Mira "hilo_reciente": identifica el concepto/término que más probablemente trabó (el que TÚ nombraste hace un momento — ej. "contribución no capturada", "carga comercial", "benchmark") y explicalo en términos simples y cotidianos.
  · Si "nivel_aclaracion" es 1 (primer intento de simplificar): usa COMO MUCHO UNA cifra INDISPENSABLE (la headline, si hace falta para anclar el ejemplo) — nada de tablas, nada de listas, ningún párrafo con 3+ cifras encadenadas.
  · Si "nivel_aclaracion" es 2 o más (el usuario TODAVÍA no entendió después de tu primera simplificación): saca TODAS las cifras — cero números — y explica con UN ejemplo concreto y cotidiano (puedes usar "un cliente" en vez de nombrar una entidad con su monto). Cambia el ÁNGULO del ejemplo respecto a tu intento anterior — no repitas la misma comparación con otras palabras.
  · Ningún término técnico sin explicar en la MISMA frase en la que lo nombras.
  · Cierra SIEMPRE con una pregunta guía concreta y accionable — nunca "¿alguna otra pregunta?" ni "¿te sirve esto?": ofrece el siguiente paso natural.
...
${mapa}
  ...
  (a) CORRECCIÓN ("no, era Lider", "te pedí ventas, no margen", "me refería al último trimestre", "te pedí del negocio y me hablas de X") → tipo="correccion" + corrige=[${REPAIR_FIELD_KEYS.join("|")}] con lo que cambió. Pon el scope corregido —si corrige el ALCANCE, normalmente level="global" y SIN filtro— y ESTA VEZ SÍ las calls que traen la respuesta corregida: nunca calls vacío. Reconoce breve y entrega. NO arrastres período, filtro, criterio ni entidad del turno anterior si dejaron de ser compatibles.
  ...
  (c) DESACUERDO — discute la INTERPRETACIÓN, no el alcance ("no creo que sea por los rebates") → tipo="desacuerdo", sin corrige: el alcance NO cambia. Vuelve a pedir la MISMA evidencia, para separar lo probado de lo indicado y lo abierto.
  (d) DATO APORTADO — afirma una cifra propia ("las ventas fueron $20M") → tipo="dato_usuario", dato={metrica,valor} tal como lo dijo, y las calls que traen LA CIFRA OFICIAL de esa métrica y alcance. Su cifra NUNCA reemplaza al dato del motor: se muestra la discrepancia. Si en ESTE turno autoriza tratarla como supuesto ("usa ese número"), aceptado=true.
...
export function buildRepairNarrateDoctrine(reparacion) {
  ...
  if (r.tipo === "correccion") {
    ...
    partes.push(`ESTE TURNO ES UN DESACUERDO: el usuario discute tu interpretación, no tu alcance. NO le des la razón sacrificando la evidencia y NO te retractes de una cifra que el motor selló. Reconoce el punto y separa explícitamente lo PROBADO (lo que el dato confirma), lo INDICADO (la señal que sostenía tu lectura) y lo ABIERTO (lo que con este dato no se puede cerrar — y di qué haría falta). Si su objeción es razonable, dilo: la honestidad acá es reconocer el límite de la evidencia, no cambiar la conclusión para complacer.`);
  } else if (r.tipo === "dato_usuario") {
    partes.push(`ESTE TURNO TRAE UNA CIFRA DEL USUARIO y NO reemplaza al dato del motor. Muestra la DISCREPANCIA con las dos cifras nombradas por su dueño ("mi dato es $X; el tuyo, $Y") y pedí la fuente, o autorización para tratarla como supuesto. Nunca la presentes como propia ni la corrijas en silencio.`);
  }
  if (Array.isArray(r.supuestos) && r.supuestos.length) {
    partes.push(`SUPUESTO APORTADO POR EL USUARIO, VIVO EN ESTA CONVERSACIÓN: ${r.supuestos.map((s) => `${s.metrica ? s.metrica + " " : ""}${s.valor}`).join(" · ")}. Es un TERCER UNIVERSO, distinto del dato del motor: (1) marcalo como SUYO en CADA lugar donde lo escribas ("según tu dato", "la cifra que aportaste") — no alcanza con decirlo una vez al principio; (2) NUNCA lo sumes, lo promedies ni lo consolides con una cifra sellada por el motor, ni lo metas dentro de un total que el producto presenta como propio; (3) todo lo que derives de él es ESCENARIO o ESTIMACIÓN, jamás un dato probado por ADI — dilo así en la misma oración.`);
  }
