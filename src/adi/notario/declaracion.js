...
function _parsearLineas(bloque) {
  ...
  for (const linea0 of limpio.split(/\r?\n/)) {
    ...
    const parsear = (t) => { const a = JSON.parse(t); if (a && typeof a === "object") { afirmaciones.push(a); return true; } errores.push(`no es un objeto: ${t.slice(0, 60)}`); return false; };
    try { parsear(linea); }
    catch (e) {
      /* comillas tipográficas («“tipo”:“orden”»): se normalizan y se reintenta; si tampoco, la línea es inválida y se dice */
      const recta = linea.replace(/[\u201C\u201D\u201E\u201F\u2033]/g, "\"").replace(/[\u2018\u2019\u201A\u201B\u2032]/g, "'");
      if (recta !== linea) { try { parsear(recta); continue; } catch { /* sigue */ } }
      errores.push(`línea inválida: ${linea.slice(0, 80)}`);
    }
  }
...
export function declaracionDeRespaldo(texto, figs, { ejesDelTenant = null, datoProyectado = null } = {}) {
  ...
  for (const f of [...parseFigures(menosAscii(s)), ..._figsSinSimbolo]) {
    ...
    /* los dueños posibles: primero los nombrados en el FRAGMENTO de la cifra (su tramo entre separadores), después los de la oración */
    const enFragmento = nombresEn(fragmento(o, pos, f.text.length));
    const nombres = enFragmento.length ? enFragmento : nombresEn(o.texto);
