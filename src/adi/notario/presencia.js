...
const _ESTADO_RE = /(?<!capital\s)(?<!del\s)(?<!el\s)(?<!de\s)(?<!d[ií]as\s)(?<!m[aá]s\s)(?<!menos\s)\b(?:inmoviliz[a-záéíóúñ]*|frenad[oa]s?|detenid[oa]s?|parad[oa]s?|bloquead[oa]s?|estancad[oa]s?|sobrestock|sobre\s+stock|sobreinventariad[oa]s?|exceso\s+de\s+stock|riesgo\s+de\s+quiebre|al\s+borde\s+del\s+quiebre|en\s+quiebre|desabastecid[oa]s?|capital\s+sano|(?:viene|vienen|est[aá]n?|sigue|siguen|queda|quedan)\s+san[oa]s?|cr[ií]tic[oa]s?(?!\s+(?:para|que|si|en\s+(?:el|la)\s+(?:lectura|decisi))\b)|sin\s+venta|sin\s+movimiento|sin\s+(?:saldo\s+)?vencido|sin\s+mora|por\s+vencer)\b/gi;
...
function _estadoCubierto(span, a) {
  if (a.tipo === "estado") { const d = estadoCanon(span); if (!_ESTADOS_DE_LA_CASA.has(d)) return true; const decl = a.estado && a.estado.estado ? estadoCanon(a.estado.estado) : ""; return _ESTADO_LO_CUBRE(d, decl); }
...
function _duenoDelPunto(s, p, nombres, ejeDe = null) {
  ...
  if (unicas.length === 1) { const e = largos[0]; const entre = e.fin <= pos ? oracion.slice(e.fin, pos) : oracion.slice(finP, e.pos); if (!/[;:—()•·,]|\s(?:y|e|pero|aunque|mientras|ni)\s|\bse\s+[a-záéíóúñ]+\s+(?:en|a)\s|\b(?:completa|alcanza|cruza|termina|queda|cae|entra|llega|aparece|concentra|est[aá])\s+en\s/.test(entre)) return unicas[0]; }
  ...
  const despues = largos.filter((e) => e.pos >= finP && e.fin <= finC && e.pos - finP <= 30 && !/\d|,|\b(?:vs\.?|contra|frente a|con|y|entre|para|que|por)\b|\bse\s+[a-záéíóúñ]+\s+en\s+/.test(oracion.slice(finP, e.pos)));
...
export function omisiones(texto, afirmaciones, { nombres = [], ejeDe = null } = {}) {
  ...
  for (const p of puntos) {
    ...
    const dueno0 = p.clase === "cifra" && p.canon ? _duenoDelPunto(s, p, nombres, ejeDe) : null;
    ...
    const cubiertaPorCanon = p.clase === "cifra" && p.canon && (() => {
      ...
      const dueno = segundo === undefined ? _duenoDelPunto(s, p, nombres, ejeDe) : segundo;
    ...
    const cubre = numeroDelOrden || mencionDeGrupo || colaDeLista || relacionDirigida || (esCola && p.canon && canonColas.has(p.canon)) || (p.clase === "cifra" ? cubiertaPorCanon : p.clase === "estado" ? envuelven.some(({ a }) => _estadoCubierto(p.span, a)) : p.clase === "relacion" ? (envuelven.some(({ a }) => _relacionCubierta(p, a, s)) || conteoRestatado(p)) : ordinalCubierto != null ? ordinalCubierto : envuelven.some(({ a }) => _CUBRE[p.clase].has(a.tipo) || (p.clase === "variacion" && a.tipo === "orden" && _METRICA_DE_VARIACION.test(String(a.metrica || "")))));   // «quién empuja el crecimiento» es un orden por la variación
