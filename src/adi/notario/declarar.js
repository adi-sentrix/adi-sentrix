...
export function crearDeclarador() {
  ...
  const D = {
    ...
    orden({ sujeto, metrica, forma, k, direccion, vs, universo, texto, textoV3, universoV3 }) { return agregar({ tipo: "orden", sujeto, metrica, orden: { forma, ...(k != null ? { k } : {}), ...(direccion ? { direccion } : {}), ...(vs ? { vs } : {}) }, ...(textoV3 ? { textoV3 } : {}), ...(universoV3 ? { universoV3 } : {}), ...(universo ? { universo } : {}), texto }); },
