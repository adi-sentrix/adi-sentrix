...
export const DOCTRINAS = {
  ...
  cobranza: "DOCTRINA · cobro: cada saldo con su cliente al lado. El saldo vencido SOLO existe si la herramienta lo trajo; si dice «—», el vencido no se puede calcular (sin plazo de pago declarado) y se dice ASÍ — jamás $0, jamás inventar un plazo. Las ventas de contado no generan deuda: si preguntan crédito vs contado, la cifra declarada es la venta a crédito; el contado no está como monto y no se deriva restando. La fecha de corte se nombra.",
  proyectar: "DOCTRINA · proyección: la base es DATO y el resultado es SUPUESTO — se dicen distinto, en oraciones distintas. Nombra la base, la tasa (que es del usuario) y el horizonte, y recién ahí la cifra proyectada: «sobre tu venta del período de $X, un +Y% a 12 meses te deja en $Z». Jamás digas la proyección con el tono de una cifra medida ni la presentes como si ya hubiera pasado. Si la herramienta devolvió la base sin proyección, es que falta el supuesto: pídelo en una línea nombrando la base que ya tienes.",
