...
// ⚠️ ACÁ VIVÍA EL GUARD SKU-MARGEN × ESCENARIO (`_esSkuMargenNoBonanza` + `_SCN_LABEL` + `_skuMargenScenarioMsg`).
// SE DESARMÓ CON EL COLAPSO DEL EJE (owner 2026-08-07, ejecutado 2026-08-30): disparaba SOLO fuera de bonanza,
// y con la base real como constante ese «fuera» dejó de existir — un guard inalcanzable no es hardening, es
// concepto muerto ocupando el camino de la respuesta. Su flag (ADI_SKU_SCENARIO_GUARD_ENABLED) se retiró con él.
// Candado: _colapso_eje_gate (definición-no-mención — este comentario puede contar la historia).
...
export function answerADI(question, context = {}, state = {}) {
  ...
  // (acá corría el guard SKU-margen×escenario — desarmado con el colapso del eje: solo disparaba non-bonanza)
