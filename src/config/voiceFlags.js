...
import { P } from "./flagProfile.js";   // perfiles de ejecucion (floor/demo/prod/dev) · resuelve features por env
...
export const ADI_QI_FILTER_ENABLED = P("ADI_QI_FILTER_ENABLED");             // ADI Core Fase 1+2 (+ Fix A/B/C muro · Cabo 1 suffix off · Cabo 2 capability-gate inventario) · default OFF · reversible · encender = poner true

// ── ADI Core · Fase 2.1 · spine (Semantic Layer + Availability Map + Intent Resolver → Validation → Planner) · default OFF ──
export const ADI_CORE_SPINE_ENABLED = P("ADI_CORE_SPINE_ENABLED");            // paraguas de TODA la Fase 2.1 · reversible · encender = poner true
export const ADI_SPINE_DIM_SUPERLATIVE_ENABLED = P("ADI_SPINE_DIM_SUPERLATIVE_ENABLED"); // 2.1a · superlativo por dimensión (marca/familia) SIN filtro · requiere el paraguas ON
export const ADI_SPINE_FILTER_ENABLED = P("ADI_SPINE_FILTER_ENABLED");          // 2.1b · filtro simple nombrado (marca/familia) SIN "por" · requiere el paraguas ON
export const ADI_SPINE_FILTER_CLARIFY_ENABLED = P("ADI_SPINE_FILTER_CLARIFY_ENABLED");  // 2.1b-2 · filtro + superlativo SIN métrica → ACLARAR · requiere ADI_SPINE_FILTER_ENABLED ON
export const ADI_SPINE_EVIDENCE_ENABLED = P("ADI_SPINE_EVIDENCE_ENABLED");        // 2.1d · cada respuesta del spine emite su evidence payload (campo hermano · texto byte-idéntico) · default OFF
export const ADI_SPINE_COMBINED_ENABLED = P("ADI_SPINE_COMBINED_ENABLED");        // 2.1c · combinado marca+cliente → AVISAR consistente (conector ampliado para/con) · requiere ADI_SPINE_FILTER_ENABLED · flag-off = 2.1b exacto

// ── ADI Core · Fase 2.2a · red de seguridad multi-turno (anti-fuga ECL-CONT + anti-contaminación) · default OFF ──
export const ADI_MT_SAFETY_ENABLED = P("ADI_MT_SAFETY_ENABLED");             // 2.2a · la continuación respeta el Availability Map (no fuga inventario) + age-check de listas plurales (no resucita contexto stale) · flag-off = piso multi-turno byte-exacto

// ── ADI Core · Fase 2.2a-2 · limpieza de foco (topic-change) + cobertura de inventario en conversación · default OFF ──
export const ADI_MT_INV_COVERAGE_ENABLED = P("ADI_MT_INV_COVERAGE_ENABLED");       // 2.2a-2 parte B · cierre SEMÁNTICO del "stock" elíptico: composeModuleOverview de inventario AVISA (no surfacea) cuando el muro está activo · compone con ADI_QI_FILTER_ENABLED
export const ADI_MT_TOPIC_CLEAN_ENABLED = P("ADI_MT_TOPIC_CLEAN_ENABLED");        // 2.2a-2 parte A · una pregunta de alcance global limpia el foco de cliente en el ORIGEN (antes de detectIntent) → cierra las 2 puertas (follow-up greedy + ECL-CONT MODO1) · preserva el follow-up anafórico

// ── ADI Core · Fase 2.2b · follow-ups del spine (resolver ACLARAR + elección tras AVISAR combinado) · default OFF ──
export const ADI_MT_SPINE_FOLLOWUP_ENABLED = P("ADI_MT_SPINE_FOLLOWUP_ENABLED");     // 2.2b · el spine escribe pendingSpineDecision al ACLARAR/AVISAR; el turno N+1 lo lee al tope (freshness + descarte por topic-change) y resuelve la respuesta suelta · compone con los flags spine

// ── ADI Core · Fase 2.2c · refinamientos deícticos (refina la vista anterior) · default OFF ──
export const ADI_MT_REFINE_METRIC_ENABLED = P("ADI_MT_REFINE_METRIC_ENABLED");      // 2.2c-1 · composeRetrieval guarda lastRetrievalContext {metric,dimension,filtro,domain}; "y por margen" (elíptico · solo métrica, SIN dimensión nueva) refina manteniendo filtro+dim · prioridad: pending del spine (2.2b) gana · compone con ADI_QI_FILTER_ENABLED
export const ADI_MT_REFINE_FILTER_ENABLED = P("ADI_MT_REFINE_FILTER_ENABLED");      // 2.2c-2 · "solo Bosch"/"y Bosch" (elíptico · marca/familia, SIN dimensión/métrica nueva) re-filtra la vista del lastRetrievalContext manteniendo métrica+dim · el QI filter decide la aplicabilidad · reusa el mecanismo de 2.2c-1
export const ADI_MT_BRAND_INV_COVERAGE_ENABLED = P("ADI_MT_BRAND_INV_COVERAGE_ENABLED"); // 2.2c-2 (cierre de fuga) · composeBrandDive omite la línea "Inventario físico $X" (L47) y AVISA el subFocus inventario cuando el Availability Map bloquea inventario · NO requiere QI_FILTER (el número de inventario es incorrecto en cualquier régimen hasta Fase 2.5) · flag-off = brand_dive del piso byte-exacto
export const ADI_MT_REFINE_CUT_ENABLED = P("ADI_MT_REFINE_CUT_ENABLED");         // 2.2c-3 (último de la Etapa 2) · "los tres peores"/"el top 3" (cuantificador elíptico N+dirección, SIN dimensión/métrica/marca nueva) acota la vista: rebana el lastRetrievalContext.ranking (top N / bottom N) → one-liner · anti-fuga: inventario → AVISA vía 2.2a (no escribe lastRetrieval) + domain-check · reusa el mecanismo de 2.2c-1
...
export const ADI_RANKING_NL_DIRECTION_ENABLED = P("ADI_RANKING_NL_DIRECTION_ENABLED");  // Capa 2 · extiende el vocab de dirección de detectRankingExtremesIntent con sinónimos naturales (worst += menor(es)/menos/bajo(s)/baja(s) · best += mayor(es)/alto(s)/alta(s)) → "los SKU con menor margen"/"mayor margen" RESPONDEN el ranking (igual que "peor"); la re-detección sobre-escribe la mala clasificación de módulo y esquiva el muro · inventario "menor rotación" lo reclama antes el spine (no se cruza)
export const ADI_CLASSIFY_SKU_COMMERCIAL_ENABLED = P("ADI_CLASSIFY_SKU_COMMERCIAL_ENABLED"); // Capa 1 (endurecimiento de raíz) · guard en resolveSemanticIntent: "sku/producto + métrica comercial (margen/contribución/ventas) y SIN señal de inventario" NO se clasifica como inventario → cae al flujo comercial · NO toca el inventario legítimo (con señal de inventario el guard no dispara) · protege también los cruces producto+comercial de la Etapa 4
export const ADI_CUADRO_OVERVIEW_ENABLED = P("ADI_CUADRO_OVERVIEW_ENABLED"); // OVERVIEW panorámico → Cuadro · "ventas/margen/rotación POR cliente/sku/..." (retrieval de dimensión, sin foco único) → buildSentrixBoleta produce una boleta lens=cuadro (dimensión+métrica del _qiContext/query_plan del motor) → botón "Ver en el Cuadro de mando" + CuadroOnlyPanel · hermano del ranking · OFF = sin boleta (byte-exacto · el overview sigue siendo solo la tabla del chat)
