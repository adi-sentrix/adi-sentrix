...
export const ADI_CUADRO_OVERVIEW_ENABLED = false; // OVERVIEW panorámico → Cuadro · "ventas/margen/rotación POR cliente/sku/..." (retrieval de dimensión, sin foco único) → buildSentrixBoleta produce una boleta lens=cuadro (dimensión+métrica del _qiContext/query_plan del motor) → botón "Ver en el Cuadro de mando" + CuadroOnlyPanel · hermano del ranking · OFF = sin boleta (byte-exacto · el overview sigue siendo solo la tabla del chat)
// ── Hardening demo · cablear mechanism_scan · default OFF ──
export const ADI_MECHANISM_SCAN_ENABLED = false; // qué mecanismos activos → composeMechanismScan (extraído, sin cablear → blanco) · OFF = blanco (piso byte-exacto)
// ── Hardening confianza · guard SKU-margen × escenario · default OFF ──
export const ADI_SKU_SCENARIO_GUARD_ENABLED = false; // margen por SKU scenario-blind → bloqueo honesto fuera de bonanza (te muestro cliente/familia) · OFF = base byte-exacto
